...
# Rho por liga (sport key → rho)
LIGA_RHO = {
    "soccer_brazil_campeonato":       0.0351,
    "soccer_brazil_serie_b":          0.0100,
    "soccer_epl":                    -0.0720,
    "soccer_germany_bundesliga":     -0.1091,
    "soccer_spain_la_liga":          -0.1091,
    "soccer_italy_serie_a":          -0.0597,
    "soccer_france_ligue_one":       -0.0232,
    # ligas sem rho definido → 0 (Poisson puro)
    "soccer_uefa_champs_league":      0.0,
    "soccer_conmebol_copa_libertadores": 0.0,
}

# Ligas que usam correção Dixon-Coles nos low-score
DIXON_COLES_LIGAS = {
...
def dc_tau(h: int, a: int, lam_h: float, lam_a: float, rho: float) -> float:
    """
    Fator de correção Dixon-Coles τ para placares baixos (0x0, 1x0, 0x1, 1x1).
    Para demais placares τ = 1.0.
    """
    if h == 0 and a == 0:
...
def build_poisson_matrix(lambda_home: float, lambda_away: float,
                         max_goals: int = 10, rho: float = 0.0,
                         use_dc: bool = False):
    """
    Matriz [home_goals][away_goals] com probabilidades.
    Se use_dc=True aplica correção Dixon-Coles nos placares 0x0, 1x0, 0x1, 1x1.
    Rho é aplicado em todos os casos (ajuste de baixo placar via τ se use_dc,
    ou embutido apenas na matriz caso contrário).
    Após correção DC, a matriz é renormalizada para somar 1.
    """
    matrix = []
...
def get_rho_and_dc(sport_key: str):
    """Retorna (rho, use_dc) para a liga selecionada."""
    rho    = LIGA_RHO.get(sport_key, 0.0)
    use_dc = sport_key in DIXON_COLES_LIGAS
    return rho, use_dc
...
def render_matrix(game_data, sport_key=""):
    if not game_data:
        matrix_ph.info("Selecione um confronto para ver a matriz.")
        return

    rho, use_dc = get_rho_and_dc(sport_key)
    lam_h, lam_a = implied_lambdas(game_data["home"], game_data["away"])
    matrix = build_poisson_matrix(lam_h, lam_a, max_goals=10, rho=rho, use_dc=use_dc)
    hw, dr, aw = extract_1x2_from_matrix(matrix)

    dc_badge = " | <span style='color:#e3b341'>Dixon-Coles ✓</span>" if use_dc else ""
    rho_str  = f" | <span style='color:#8b949e'>ρ = {rho:+.4f}</span>" if rho != 0 else ""
    ...
    <div style='margin-bottom:10px;font-family:Share Tech Mono,monospace;font-size:13px'>
      ...
      <span style='color:#f85149'>λ Fora: <b>{lam_a}</b></span>{rho_str}{dc_badge}<br>
...
def render_cs(game_data, sport_key=""):
    if not game_data:
...
render_matrix(selected_data, sport_key=sport)
render_cs(selected_data, sport_key=sport)
